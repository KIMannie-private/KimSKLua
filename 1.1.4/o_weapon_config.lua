...
local function load_config(config_idx)
    if active_config_idx ~= config_idx then
        ...
        ui.set(label_ref, "[ KIM ] Active Weapon: " .. config_idx_to_name[config_idx])
    end
end
...
local function init()
    ...
    enable_ref = ui.new_checkbox("RAGE", "Other", "[ KIM ] Adaptive config")
    config_ref = ui.new_combobox("RAGE", "Other", "\nAdaptive config", config_idx_to_name)
    label_ref = ui.new_label("RAGE", "Other", "[ KIM ] Active Weapon: " .. ui.get(config_ref))
    ...
    duplicate("RAGE", "Aimbot", "Force safe point on limbs", ui.new_checkbox)
