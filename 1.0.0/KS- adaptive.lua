...
local controls = {
    ...
    visible         = false,
    enabled         = ui_new_checkbox( "LUA", "B", "[ KIM ] Adaptive weapons" ),
    selected_weapon = ui_new_combobox( "LUA", "A", "[ KIM ] Weapons", get_items( weapon_indexes ) ),
    ...
    indicators      = ui.new_label("RAGE", "Aimbot", "o───────────────────────────────o"),
    key_checkbox = ui.new_hotkey("Rage", "Other", "[ KIM ] Resolver LegitAA", false),
    ...
    indicators      = ui.new_label("AA", "anti-aimbot angles", "o───────────────────────────────o"),
    key_damage      = ui_new_hotkey( "LUA", "B", "[ KIM ] Damage override", false ),
    key_hitbox      = ui_new_hotkey( "LUA", "B", "[ KIM ] Hitbox override", false ),
    key_head        = ui_new_hotkey( "LUA", "B", "[ KIM ] Force head", false ),
    indicators      = ui_new_checkbox("LUA", "B", "[ KIM ] Indicators" ),
...
local function generate_weapon_controls( )
    for name in pairs( weapon_indexes ) do 
        weapon_info[ name ] = {
            selection           = ui_new_combobox( "LUA", "A", s_format( "[%s] Target selection", name ), { "Cycle","Cycle (2x)", "Near crosshair", "Highest damage", "Lowest ping", "Best K/D ratio", "Best hit chance" } ),
            hitbox              = ui_new_multiselect( "LUA", "A", s_format( "[%s] Target hitbox", name ), { "Head", "Chest", "Stomach", "Arms", "Legs", "Feet" } ),
            multipoint          = ui_new_multiselect( "LUA", "A", s_format( "[%s] Multi-point", name ), { "Head", "Chest", "Stomach", "Arms", "Legs", "Feet" } ),
            multipoint_scale    = ui_new_slider( "LUA", "A", s_format( "[%s] Multi-point scale", name ), 24, 100, 50, true, "%", 1, labels.multipoint),
            prefersafe          = ui_new_checkbox( "LUA", "A", s_format( "[%s] Prefer safe point", name ) ),
            forcesafe_limbs     = ui_new_checkbox("LUA", "A", s_format( "[%s] Force safe point on limbs", name ) ),
            forcesafe           = ui_new_combobox( "LUA", "A", s_format( "[%s] Force safe point", name ), { "On hotkey", "Toggle", "Always on", "Toggle" } ),
            hit_chance          = ui_new_slider( "LUA", "A", s_format( "[%s] Minimum hit chance", name ), 0, 100, 55, true, "%", 1, labels.hit_chance ),
            damage              = ui_new_slider( "LUA", "A", s_format( "[%s] Minimum damage", name ), 0, 124, 15, true, "\n", 1, labels.damage ),
            boost               = ui_new_combobox( "LUA", "B", s_format( "[%s] Accuracy boost", name ), { "Off", "Low", "Medium", "High", "Maximum" } ),
            delay               = ui_new_checkbox( "LUA", "B", s_format( "[%s] Delay shot", name ) ),
            stop                = ui_new_checkbox( "LUA", "B", s_format( "[%s] Quick stop", name ) ),
            stop_options        = ui_new_multiselect( "LUA", "B", s_format( "[%s] Quick stop options", name ), { "Early", "Slow motion", "Duck", "Move between shots", "Ignore molotov" } ),
            baim_peek           = ui_new_checkbox( "LUA", "B", s_format( "[%s] Force body aim on peek", name ) ),
            baim                = ui_new_checkbox( "LUA", "B", s_format( "[%s] Prefer body aim", name ) ),
            baim_disablers      = ui_new_multiselect( "LUA", "B", s_format( "[%s] Prefer body aim disablers", name ), { "Low inaccuracy","Target shot fired","Target resolved","Safe point headshot","Low damage" } ),
            onshot              = ui_new_checkbox( "LUA", "B", s_format( "[%s] On shot anti-aim", name ) ),
            doubletap           = ui_new_checkbox( "LUA", "B", s_format( "[%s] Double tap", name ) ),
            doubletap_stop      = ui_new_multiselect( "LUA", "B", s_format( "[%s] Double tap quick stop", name ), { "Slow motion", "Duck", "Move between shots" } ),
            overrides           = ui_new_multiselect( "LUA", "B", s_format( "[%s] Extra's", name ), { "Override hitbox", "Override damage", "Visible damage", "No-spread fix", "Double tap" } ),
            hitbox_override     = ui_new_multiselect( "LUA", "B", s_format( "[%s] Target hitbox override", name ), {"Head", "Chest", "Stomach", "Arms", "Legs", "Feet"} ),
            visible             = ui_new_slider( "LUA", "B", s_format( "[%s] Visible minimum damage", name ), 0, 124, 15, true, "\n", 1, labels.damage ),
            damage_override     = ui_new_slider( "LUA", "B", s_format( "[%s] Override minimum damage", name ), 0, 124, 15, true, "\n", 1, labels.damage ),
            hit_chance_air      = ui_new_slider( "LUA", "B", s_format( "[%s] No-spread fix hit chance", name ), 0, 100, 30, true, "%", 1, labels.hit_chance ),
            damage_air          = ui_new_slider( "LUA", "B", s_format( "[%s] No-spread fix minimum damage", name ), 0, 124, 20, true, "\n", 1, labels.damage ),
            doubletap_hb        = ui_new_multiselect( menu[ 1 ],  menu[ 2 ], s_format( "[%s] Double tap hitbox", name ), { "Head", "Chest", "Stomach", "Arms", "Legs", "Feet" } ),
            doubletap_hc        = ui_new_slider( "LUA", "B", s_format( "[%s] Double tap minimum hit chance", name ), 0, 100, 55, true, "%", 1, labels.hit_chance ),
            doubletap_dmg       = ui_new_slider( "LUA", "B", s_format( "[%s] Double tap minimum damage", name ), 0, 124, 15, true, "\n", 1, labels.damage )
 		}
    end
end
...
local function menu_callback( e, menu_call )
    ...
    multi_exec( ui_visible, {
        ...
        [ controls.key_checkbox  ] = not state
    } )
end
...
local function update_settings( weapon )
    ...
    for name, ref in pairs( reference ) do
        ...
        if name == "checkbox" then
            if ui_get( controls.key_checkbox ) then
                ui_set( ref, "Legit" )
            elseif ui_get( controls.key_checkbox ) and contains( ui_get( active.key_checkbox ), "checkbox" ) then
                ui_set( ref, ui_get( active.key_checkbox ) )
            end
        end
...
local function draw_indicators( )
    ...
    if ui_get( controls.key_checkbox ) then
        renderer_indicator( 255, 50, 50, 255, "Resolver AA" )
    end
    if ui_get( controls.key_head ) then
    ...
    if ui_get( controls.key_damage ) and contains( ui_get( temp.overrides ), "Override damage" ) then
        client.draw_indicator(c, 50,255,255, 255, "" .. ui.get(refs.md))
    end
...
require("./♛KIM-Beta♛/KIM/KS- Weapon perfer")

local err = client_set_event_callback('draw_indicators', draw_indicators)
        end
        ...
        local function onLoad()
            if(authed)then
                client.log("[" .. luaName .. "]Authed, welcome")
            else
                client.log("[" .. luaName .. "]You can't use this lua, sorry")
            end
        end

        client.delay_call(3,onLoad)
