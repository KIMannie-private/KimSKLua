...
"州边鄙而退，岂力能制敌欤？盖蒙其伤怜而已！诸史何为忍苦征敛！故作诗一篇以示官吏。",
"昔岁逢太平，山林二十年。",
"泉源在庭户，洞壑当门前。",
"井税有常期，日晏犹得眠。",
"忽然遭时变，数岁亲戎旃。",
...
"不寝听金钥，因风想玉珂。",
"明朝有封事，数问夜如何？",
"杜甫：至德二载甫自京金光门出，问道归凤翔。乾元初从左拾遗移华州掾。与亲",
"故别，因出此门。有悲往事。",
"此道昔归顺，西郊胡正繁。",
"至今残破胆，应有未招魂。",
"近得归京邑，移官岂至尊？",
...
"惟将终夜长开眼，报答平生未展眉。",
"白居易：自河南经乱，关内阻饥，兄弟离散，各在一处。因望月有感，聊书所怀",
...
local num_spamline = get_table_length(spamline)


local trashspam = ui.new_checkbox("MISC", "Miscellaneous", "tangshi spam")
local trashstop = ui.new_checkbox("MISC", "Miscellaneous", "tangshi spam non-stop")



local function handle_menu()
    if ui_get(trashspam) == true then
        ui_set_visible(trashstop, true)
    else
        ui_set_visible(trashstop, false)
    end
end
handle_menu()
ui_set_callback(trashspam, handle_menu)
...
local function on_paint(ctx)
	if not ui_get(trashspam) then
		if ui_get(trashstop) then
			ui_set(trashstop, false)
		end
		return
	end
	if i <= num_spamline then
		if globals_tickcount() > lasttime then
			local commandtrashtalk = "say " .. spamline[i]
			console_cmd(commandtrashtalk)
			lasttime = globals_tickcount() + 50
			i = i + 1
		end
	end
	if not ui_get(trashstop) then
		if i > num_spamline then
			ui_set(trashspam, false)
			i = 1
		end
	end
	if ui_get(trashstop) then
