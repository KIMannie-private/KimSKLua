...
local offset_x, offset_y = -1785, 1020
-- interfaces

local screen_size = { client_screen_size() }

local offset_x_bar = ui.new_slider("Visuals", "Effects", "Watermark_X_Offset",54,100,1)
local offset_y_bar = ui.new_slider("Visuals", "Effects", "Watermark_Y_Offset",95,100,1)
...
local renderer_container = function( x, y, w, h)
...
local fps = accumulate_fps()
  ...
  local x_,y_ = client.screen_size()

local x = x_ * (ui.get(offset_x_bar)/100)
local y = y_ * (ui.get(offset_y_bar)/100)
...
if ui_get(menu.watermark) then
  ...
  local pos = {x - size[1] + 20, x - size[1]+size[1]/2-60, y+4 }

  text(pos[2],pos[3],"a",string_upper("k i m  private [ - B e t a - ]"))
