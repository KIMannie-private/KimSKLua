...
local aimbotlog_enable = ui.new_checkbox("Rage", "Other", "[ KIM ] Aim Log")
local on_fire_enable = ui.new_checkbox("Rage", "Other", "[ KIM ] Fire")
local on_fire_colour = ui.new_color_picker("Rage", "Other", "[ KIM ] Fire", 140, 217, 255, 255)
local on_miss_enable = ui.new_checkbox("Rage", "Other", "[ KIM ] Miss")
local on_miss_colour = ui.new_color_picker("Rage", "Other", "[ KIM ] Miss", 255, 145, 145, 255)
local on_damage_enable = ui.new_checkbox("Rage", "Other", "[ KIM ] DMG")
local on_damage_colour = ui.new_color_picker("Rage", "Other", "[ KIM ] DMG", 255, 215, 145, 255)
...
local function on_aim_fire(e)
    if ui_get(aimbotlog_enable) and ui_get(on_fire_enable) and e ~= nil then
        ...
        "Fired at ", target_name,
        "  in the ", group,
        "  for ", e.damage,
        "  damage, HC: ", string.format("%d", e.hit_chance),
...
local function on_player_hurt(e)
    ...
    local message = "Hit " .. target_name .. " in the " .. group .. " for " .. e.dmg_health .. " damage  HP: " .. rem_health
    if rem_health <= 0 then
...
local function on_aim_miss(e)
    ...
    if e.reason == "?" then
        ...
        client_color_log(r, g, b,
        "Missed shot due to ", target_name,
        ", HB: ", group,
        "  R: ", reason)
    end
end
